# Document: https://docs.docker.com/develop/develop-images/dockerfile_best-practices/
...
Refer to Processes under The Twelve-factor App
...
instance, Celery can spawn multiple worker
processes, and Apache can create one process per
request.
...
GitHub Actions or another CI/CD pipeline to
...
images. Docker recommends the Alpine image as it
...
See also
LABEL in the Dockerfile reference.
...
This script uses the exec builtin so that the final running application becomes the container's PID 1. This allows the application to receive any Unix signals sent to the container. For more information, see the
...
Due to an unresolved bug in the
...
running it as non-root, consider using âgosuâ.
...
Dockerfile, as you can see in Rubyâs ONBUILD variants.
